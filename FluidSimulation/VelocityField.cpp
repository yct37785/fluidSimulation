...
VelocityField::VelocityField()
{
	...
	// face centers: + 0.5
	u = new QuantityField(p_GridInfo->xCellsCount + 1, p_GridInfo->yCellsCount, 0.f, Hoffset, uMarker);
	v = new QuantityField(p_GridInfo->xCellsCount, p_GridInfo->yCellsCount + 1, Hoffset, 0.f, vMarker);
}
...
void VelocityField::advectSelf_semiLagrangian(float t, std::unordered_map<int, int>& liquidCells)
{
	...
	for (int y = 0; y < p_GridInfo->yCellsCount + 1; ++y)
	{
		for (int x = 0; x < p_GridInfo->xCellsCount + 1; ++x)
		{
			// x
			if (y < p_GridInfo->yCellsCount)
			{
			...
			// no slip
			if (x == 0 || x == p_GridInfo->xCellsCount)
				u->setQuantityAtIdx(x, y, 0.f);
			// y
			if (x < p_GridInfo->xCellsCount)
			{
			...
			// no slip
			if (y == 0 || y == p_GridInfo->yCellsCount)
...
void VelocityField::applyExternalForces(float t, std::unordered_map<int, int>& liquidCells)
{
	...
	for (int y = 0; y < p_GridInfo->yCellsCount + 1; ++y)
	{
		...
		{
			if (liquidCells.count((y - 1) * p_GridInfo->xCellsCount + x) || liquidCells.count(y * p_GridInfo->xCellsCount + x))
			{
...
void VelocityField::extrapolate(std::unordered_map<int, int>& liquidCells)
{
	for (int y = 0; y < p_GridInfo->yCellsCount; ++y)
	{
		for (int x = 0; x < p_GridInfo->xCellsCount; ++x)
		{
			...
			if (x > 0)
			{
				if (!liquidCells.count(y * p_GridInfo->xCellsCount + x - 1) && !liquidCells.count(y * p_GridInfo->xCellsCount + x))
